...
import config
from RamoMusic import LOGGER, app, userbot
from RamoMusic.core.call import Mody
from RamoMusic.misc import sudo
from RamoMusic.plugins import ALL_MODULES
from RamoMusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("RamoMusic.plugins" + all_module)
    LOGGER("RamoMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Mody.start()
    try:
        await Mody.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("RamoMusic").error(
    ...
    await userbot.stop()
    await azkar()
    LOGGER("RamoMusic").info("Stopping Ramo Music Bot...")
